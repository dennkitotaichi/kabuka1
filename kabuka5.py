...
## 終値を時系列にプロット
plt.title('日経平均株価（終値）の推移nikkeiheikinnkabukanosuii')
plt.plot(range(len(closing_price)), closing_price)
plt.show()

plt.title('日経平均株価（終値）の推移　過去300日分のみnikkeiheikinnkabuka nosuii kako300nitibunn')
plt.plot(range(len(closing_price[-300:])), closing_price[-300:])
plt.show()

plt.title('日経平均株価（終値）の推移　過去30日分のみnikkeiheikinnkabuka nosuii kako30nitibunn')
...
plt.plot(pre_date, df_predict['予測値'].values, 'r', alpha=0.6, marker='.', label="予測値 yosokuti", linewidth=1)
plt.xticks(rotation=70)
plt.legend()
plt.grid(True)
plt.show()
